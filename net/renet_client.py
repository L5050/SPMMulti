...
- base:
    ...
    - CMD_MAP_INTERACT: Command to interact with a map object (expects a string map_obj)
- effect:
    - CMD_EFFECT_TEST: A test command to trigger a visual or audio effect in the game (arguments TBD)
"""
CATEGORIES = {
    "reader": 0x00,
    "base": 0x01,
...
def start_gui():
    ...
    def setup_packet():
        ...
        command = 0x99
        try:
            header = struct.pack(">BBH", category, command, 4)
            return header
        
        except Exception as e:
            client.log(f"Failed to generate packet: {e}")
    ...
    def update_layout(event=None):
        ...
        log_box.place_configure(relx=0, rely=1.0, x=10,y=-10, height=window_height//3, width=window_width-500, anchor='sw')
